...
Error Code: 3. Error writing file 'C:\WINDOWS\SERVIC~1\NETWOR~1\AppData\Local\Temp\MYfcrcqtsknkhwy8c9' (OS errno 28 - No space left on device)

--Aggregate TABLE
CREATE VIEW table_agregate as
WITH base_data AS (
    SELECT
        p.id_invoice,
        pl.nama,
...
SELECT
    ...
    bd.harga,
    bd.total_harga,
    ipl.total_transaksi_pelanggan,
    ipl.total_pembelian_pelanggan,
    ipl.AVG_jumlah_pembelian_pelanggan,
    ipr.total_produk_terjual,
    ipr.total_pendapatan_produk,
    ipr.AVG_jumlah_pembelian_produk,
    mr.periode,
    mr.total_pendapatan,
...
,insight_pelanggan ipl, insight_produk ipr, total_keseluruhan tk
ORDER BY bd.cabang_sales, DATE_FORMAT(bd.tanggal, '%Y-%m'), bd.nama_barang;

WHERE DATE_FORMAT(bd.tanggal, '%Y-%m') = '2022-01' and bd.cabang_sales = 'bandung'
ORDER BY bd.cabang_sales, DATE_FORMAT(bd.tanggal, '%Y-%m'), bd.nama_barang
limit 5;
...
WHERE DATE_FORMAT(bd.tanggal, '%Y-%m') = '2022-01'
ORDER BY bd.cabang_sales, DATE_FORMAT(bd.tanggal, '%Y-%m'), bd.nama_barang
limit 5;
limit 5;WHERE DATE_FORMAT(bd.tanggal, '%Y-%m') = '2022-01' and bd.cabang_sales = 'bandung'
ORDER BY bd.cabang_sales, DATE_FORMAT(bd.tanggal, '%Y-%m'), bd.nama_barang
limit 5;
